...
pub fn find_best_child_edge<'a, R>(c: &ChildList<'a>, player: game::PlayerMarker, epoch: usize,
                                   explore_bias: f64, rng: &mut R)
                                   -> ::std::result::Result<usize, UcbError> where R: Rng {
    if c.is_empty() {
        return ::std::result::Result::Err(UcbError::NoChildren)
    }
    let log_parent_visits =
                ...
                match score(log_parent_visits, stats.visits as f64,
                    ...
                    Result::Value(ucb) => {
                        match ucb.partial_cmp(&best_ucb) {
                            None =>
                                return ::std::result::Result::Err(UcbError::InvalidComputation),
