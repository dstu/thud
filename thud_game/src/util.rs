...
pub fn cmp_actions(a: &Action, b: &Action) -> Ordering {
  match (*a, *b) {
    ...
    (Action::Shove(_, _, _, _), Action::ProposeEnd) => Ordering::Less,
    (Action::Shove(_, _, _, _), Action::HandleEndProposal(_)) => Ordering::Less,
    (Action::Shove(_, _, _, _), _) => Ordering::Greater,

    (Action::ProposeEnd, Action::ProposeEnd) => Ordering::Equal,
    (Action::ProposeEnd, Action::HandleEndProposal(_)) => Ordering::Less,
    (Action::ProposeEnd, _) => Ordering::Greater,

    (Action::HandleEndProposal(r1), Action::HandleEndProposal(r2)) => r1.cmp(&r2),
    (Action::HandleEndProposal(_), _) => Ordering::Greater,
  }
}
