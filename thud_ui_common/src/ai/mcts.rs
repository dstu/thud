...
impl Handle {
    pub fn new<R>(rng: R, exploration_bias: f64) -> Self where R: 'static + Rng + Send {
        ...
        let join_handle = {
            ...
            thread::spawn(move || {
                ...
                while let Instruction::Search { state, iteration_count, simulation_count } = parameters_rx.recv().unwrap() {
                    // Initialize search.
                    *status.lock().unwrap() = Status::Initializing;
                    ...
                    if !error {
                        ...
                        let statistics =
                            if let Status::Working { ref statistics, .. } = *status {
                                statistics.clone()
                            } else {
