...
fn main() {
  ...
  let move_selection_criterion = match matches
    .value_of(thud_ui_common::MOVE_SELECTION_CRITERION_FLAG)
    .map(|x| x.parse::<thud_ui_common::MoveSelectionCriterion>())
  {
    Some(Ok(x)) => x,
    Some(Err(e)) => panic!("Bad move selection criterion: {}", e),
    None => thud_ui_common::MoveSelectionCriterion::VisitCount,
  };
  let rng = match matches
    .value_of(thud_ui_common::RNG_SEED_FLAG)
    .map(|x| x.parse::<u32>())
  {
  ...
  let mut search_state =
    mcts::SearchState::<IsaacRng, thud_ui_common::ThudGame>::new(rng, exploration_bias);
  ...
  while !state.terminated() {
    ...
    for iteration in 0..iteration_count {
      ...
      match search_state.search(&mut graph, &state, |_: usize| mcts::SearchSettings {
        ...
        Ok(stats) => {
          ...
          if iteration % 1000 == 0 || iteration + 1 == iteration_count {
            ...
            for actions in stats.into_iter() {
              info!(
                "{:?}: {:?}; UCB = {:?}",
                actions.action, actions.payoff, actions.ucb
              );
              ...
              best_action = match (best_action, move_selection_criterion) {
                ...
                (Some(_), thud_ui_common::MoveSelectionCriterion::VisitCount)
                  if best_visits < payoff_visits =>
                {
                  best_visits = payoff_visits;
                  Some(actions.action)
                }
                (Some(_), thud_ui_common::MoveSelectionCriterion::Ucb) => match actions.ucb {
